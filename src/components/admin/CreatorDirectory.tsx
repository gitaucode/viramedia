...
export default function CreatorDirectory(){
  ...
  const load=useCallback(async()=>{
    ...
    if(!r.ok){
      if(j.code==="DB_NOT_CONFIGURED"){setDbMissing(true);setAuth("ready");return}
      setError(j.error||"Could not load creators");return;
    }
    setCreators(j.creators||[]);setDbMissing(false);setAuth("ready");
    const cr=await fetch("/api/admin/campaigns",{cache:"no-store"});
    if(cr.ok){const cj=await cr.json();setCampaigns(cj.campaigns||[])}
  },[q,status,city]);
  ...
  async function login(e:FormEvent){
    ...
    setPassword("");setAuth("checking");await load();
  }
  async function updateCreator(id:number,patch:Record<string,string>){
    const r=await fetch("/api/admin/creators",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id,...patch})});
    if(r.ok){await load();setSelected(s=>s&&s.id===id?{...s,...patch}:s)}
  }
  async function shortlistCreator(creatorId:number,campaignId:number){
    const r=await fetch("/api/admin/campaign-creators",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"add",creatorId,campaignId,status:"shortlisted"})});
    if(!r.ok){const j=await r.json().catch(()=>({}));setError(j.error||"Could not shortlist creator");return}
    await load();
  }
  ...
  if(auth==="login")return <main className="admin-shell admin-centre"><div className="login-theme"><ThemeToggle/></div><form className="admin-login" onSubmit={login}><div className="admin-brand">VIRA <span>OPS</span></div><h1>Welcome back.</h1><p>Sign in to manage creators and campaign selections.</p><label>Admin password<input type="password" value={password} onChange={e=>setPassword(e.target.value)} autoFocus required/></label><button>Sign in →</button>{error&&<p className="admin-error">{error}</p>}</form></main>;
