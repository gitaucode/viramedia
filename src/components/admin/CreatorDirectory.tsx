...
export default function CreatorDirectory(){
 ...
 async function logout(){await fetch('/api/admin/logout',{method:'POST'});setCreators([]);setShortlists([]);setSelected(null);setAuth('login')}
 ...
 if(auth==='login')return <main className="admin-shell admin-centre"><form className="admin-login" onSubmit={login}><div className="admin-brand">VIRA <span>ADMIN</span></div><h1>Welcome back.</h1><p>Sign in to manage creators and campaign shortlists.</p><label>Admin password<input type="password" value={password} onChange={e=>setPassword(e.target.value)} autoFocus required/></label><button>Sign in →</button>{error&&<p className="admin-error">{error}</p>}</form></main>;

 return <main className="admin-shell">
  <nav className="admin-nav"><div className="admin-nav-inner"><div className="admin-brand">VIRA <span>ADMIN</span></div><div className="admin-nav-links"><a className="active" href="/admin/creators">Creators</a><a href="#shortlists">Campaigns</a></div><div className="admin-nav-actions"><a href="/" target="_blank" rel="noreferrer">View site ↗</a><button onClick={()=>void logout()}>Log out</button></div></div></nav>
  <div className="admin-workspace">
   <header className="admin-top"><div><span className="admin-kicker">CREATOR OPERATIONS</span><h1>Creator Directory</h1><p>Search the network, review applications and build campaign shortlists.</p></div><div className="admin-count"><strong>{creators.length}</strong><span>creators shown</span></div></header>
   ...
   <aside className="shortlist-panel" id="shortlists"><div className="panel-head"><span>CAMPAIGNS</span><h2>Shortlists</h2><p>Group creators around an active brief.</p></div><form onSubmit={createShortlist}><input value={newShortlist} onChange={e=>setNewShortlist(e.target.value)} placeholder="Campaign name"/><button>Create</button></form>{shortlists.map(s=><div className="shortlist-row" key={s.id}><div><strong>{s.name}</strong><span>{s.creator_count} creators</span></div><b>→</b></div>)}{!shortlists.length&&<div className="shortlist-empty"><p>No shortlists yet.</p><span>Create one when a client brief comes in.</span></div>}</aside></div></>}
  </div>
