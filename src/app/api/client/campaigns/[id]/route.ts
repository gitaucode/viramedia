...
export async function GET(_:Request,{params}:{params:Promise<{id:string}>}){
 ...
 const campaign=await db.prepare(`SELECT camp.id,camp.name,camp.client,camp.primary_client_id,camp.client_objective,camp.report_summary,camp.report_insights,camp.report_recommendations,camp.status,camp.start_date,camp.end_date FROM campaign_clients cc JOIN campaigns camp ON camp.id=cc.campaign_id WHERE cc.client_id=? AND camp.id=?`).bind(client.id,campaignId).first();
...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
 const client=await getClientSession();if(!client)return NextResponse.json({error:'Unauthorized'},{status:401});
 const db=getOpsDb();if(!db)return NextResponse.json({error:'Database unavailable'},{status:503});
 try{const {id}=await params,campaignId=Number(id),b=await request.json(),deliverableId=Number(b.deliverableId),status=String(b.status||'');if(!Number.isInteger(campaignId)||!Number.isInteger(deliverableId)||!allowed.has(status))return NextResponse.json({error:'Invalid review'},{status:400});const access=await db.prepare(`SELECT d.id,d.title,d.creator_id,camp.name campaign_name,c.full_name creator_name,c.email creator_email FROM campaign_clients cc JOIN deliverables d ON d.campaign_id=cc.campaign_id JOIN campaigns camp ON camp.id=d.campaign_id LEFT JOIN creators c ON c.id=d.creator_id WHERE cc.client_id=? AND d.campaign_id=? AND d.id=? AND d.client_approval_status='awaiting_client'`).bind(client.id,campaignId,deliverableId).first<{id:number;title:string;creator_id:number|null;campaign_name:string;creator_name:string|null;creator_email:string|null}>();if(!access)return NextResponse.json({error:'Not available for review'},{status:403});const feedback=clean(b.feedback);if(status==='changes_requested'&&!feedback)return NextResponse.json({error:'Add feedback before requesting changes'},{status:400});
 ...
 await recordActivity({actorType:'client',actorId:client.id,campaignId,creatorId:access.creator_id,clientId:client.id,deliverableId,eventType:status==='approved'?'deliverable.client_approved':'deliverable.client_changes_requested',title:status==='approved'?'Client approved content':'Client requested changes',detail:`${client.company} reviewed ${access.title}.`,metadata:{status,feedback}});
