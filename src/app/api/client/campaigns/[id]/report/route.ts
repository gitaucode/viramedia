...
const pdfSafe=(v:unknown)=>String(v??'').replace(/[\u2018\u2019]/g,"'").replace(/[\u201C\u201D]/g,'"').replace(/[\u2013\u2014]/g,'-').replace(/[^\x20-\x7E\xA0-\xFF]/g,'?');
...
export async function GET(_:Request,{params}:{params:Promise<{id:string}>}){
 ...
 const campaign=await db.prepare(`SELECT camp.id,camp.name,camp.client,camp.client_objective,camp.report_summary,camp.report_insights,camp.report_recommendations,camp.status,camp.start_date,camp.end_date FROM campaign_clients cc JOIN campaigns camp ON camp.id=cc.campaign_id WHERE cc.client_id=? AND camp.id=?`).bind(client.id,campaignId).first<Record<string,unknown>>();
 if(!campaign)return new Response('Not found',{status:404});
 const r=await db.prepare(`SELECT d.title,d.submission_url,d.client_approval_status,c.full_name creator_name,COALESCE(pm.views,0) views,COALESCE(pm.reach,0) reach,COALESCE(pm.impressions,0) impressions,COALESCE(pm.likes,0) likes,COALESCE(pm.comments,0) comments,COALESCE(pm.shares,0) shares,COALESCE(pm.saves,0) saves,COALESCE(pm.clicks,0) clicks,COALESCE(pm.conversions,0) conversions,COALESCE(pm.spend,0) spend FROM deliverables d LEFT JOIN creators c ON c.id=d.creator_id LEFT JOIN performance_metrics pm ON pm.deliverable_id=d.id WHERE d.campaign_id=? AND d.client_approval_status='approved' ORDER BY d.id`).bind(campaignId).all<Record<string,unknown>>();
 const rows=r.results||[];
 const totals=rows.reduce<MetricTotals>((a,x)=>({views:a.views+n(x.views),reach:a.reach+n(x.reach),impressions:a.impressions+n(x.impressions),likes:a.likes+n(x.likes),comments:a.comments+n(x.comments),shares:a.shares+n(x.shares),saves:a.saves+n(x.saves),clicks:a.clicks+n(x.clicks),conversions:a.conversions+n(x.conversions),spend:a.spend+n(x.spend)}),{views:0,reach:0,impressions:0,likes:0,comments:0,shares:0,saves:0,clicks:0,conversions:0,spend:0});
 ...
 const wrap=(value:string,max=82)=>{const words=pdfSafe(value).replace(/\s+/g,' ').trim().split(' '),lines:string[]=[];let line='';for(const word of words){const test=(line+' '+word).trim();if(test.length>max&&line){lines.push(line);line=word}else line=test}if(line)lines.push(line);return lines};
 ...
 const label=(value:string)=>{ensure(24);page.drawText(pdfSafe(value.toUpperCase()),{x:52,y,size:8,font:bold,color:pink});y-=20};
 ...
 text(pdfSafe(campaign.name||'Campaign'),27,{bold:true,max:36,gap:4});text(pdfSafe(campaign.client||client.company),12,{color:muted,gap:18});
 label('Campaign overview');text(pdfSafe(campaign.client_objective||'Campaign objective not yet added.'),11,{max:86,gap:12});text(`${pdfSafe(campaign.start_date||'Start TBD')} - ${pdfSafe(campaign.end_date||'End TBD')}`,9,{color:muted,gap:20});
 ...
 if(pdfSafe(campaign.report_summary).trim()){label('Executive summary');text(pdfSafe(campaign.report_summary),10,{max:90,gap:18})}
 label('Content performance');
 if(!rows.length)text('No client-approved campaign content has been added to this report yet.',10,{color:muted,gap:12});
 for(const row of rows){ensure(104);page.drawLine({start:{x:52,y},end:{x:543,y},thickness:.6,color:rgb(.82,.82,.82)});y-=18;text(pdfSafe(row.title||'Deliverable'),12,{bold:true,gap:1,max:55});text(pdfSafe(row.creator_name||'Creator'),9,{color:muted,gap:5});text(`Views ${fmt(n(row.views))}   Reach ${fmt(n(row.reach))}   Likes ${fmt(n(row.likes))}   Comments ${fmt(n(row.comments))}   Shares ${fmt(n(row.shares))}   Clicks ${fmt(n(row.clicks))}`,8,{color:muted,max:105,gap:10})}
 if(pdfSafe(campaign.report_insights).trim()){label('Key insights');text(pdfSafe(campaign.report_insights),10,{max:90,gap:18})}
 if(pdfSafe(campaign.report_recommendations).trim()){label('Recommendations');text(pdfSafe(campaign.report_recommendations),10,{max:90,gap:18})}
 ...
 const bytes=await pdf.save(),safe=pdfSafe(campaign.name||'campaign').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,''),body=Uint8Array.from(bytes).buffer;
 return new Response(body,{headers:{'Content-Type':'application/pdf','Content-Disposition':`attachment; filename="${safe||'campaign'}-report.pdf"`,'Cache-Control':'private, no-store'}});
}
