...
export async function GET(){
 ...
 const campaigns=await db.prepare(`SELECT s.id,s.name,s.client,s.client_objective,s.status,s.start_date,s.end_date,COUNT(DISTINCT CASE WHEN d.client_approval_status!='not_ready' THEN d.id END) deliverable_count,COALESCE(SUM(CASE WHEN d.client_approval_status!='not_ready' THEN pm.views ELSE 0 END),0) views,COALESCE(SUM(CASE WHEN d.client_approval_status!='not_ready' THEN pm.reach ELSE 0 END),0) reach,COALESCE(SUM(CASE WHEN d.client_approval_status!='not_ready' THEN pm.impressions ELSE 0 END),0) impressions,COALESCE(SUM(CASE WHEN d.client_approval_status!='not_ready' THEN pm.likes+pm.comments+pm.shares+pm.saves ELSE 0 END),0) engagements FROM campaign_clients cc JOIN shortlists s ON s.id=cc.campaign_id LEFT JOIN deliverables d ON d.campaign_id=s.id LEFT JOIN performance_metrics pm ON pm.deliverable_id=d.id WHERE cc.client_id=? GROUP BY s.id ORDER BY CASE WHEN s.status='active' THEN 0 WHEN s.status='planning' THEN 1 ELSE 2 END,datetime(s.created_at) DESC`).bind(client.id).all();
