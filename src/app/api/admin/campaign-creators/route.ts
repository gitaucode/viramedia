...
import { addCampaignCreator,getOpsDb,listCampaignCreators,removeCampaignCreator } from "@/lib/ops-db";
...
export async function POST(request:Request){
  ...
  try{
    ...
    const campaign=await db.prepare('SELECT id,name,client FROM shortlists WHERE id=?').bind(campaignId).first<{id:number;name:string;client:string}>();
    ...
    if(b.action==='remove'){
      ...
      await recordActivity({
        ...
        campaignId,
        creatorId,
        eventType:'campaign_creator.removed',
        title:'Creator removed',
        detail:`${creator.full_name} was removed from ${campaign.name}.`,
      });
    ...
    const existing=await db.prepare('SELECT 1 ok FROM shortlist_creators WHERE shortlist_id=? AND creator_id=?').bind(campaignId,creatorId).first<{ok:number}>();
    await addCampaignCreator(campaignId,creatorId);
    if(!existing){
      await recordActivity({
        actorType:'admin',
        campaignId,
        creatorId,
        eventType:'campaign_creator.assigned',
        title:'Creator assigned',
        detail:`${creator.full_name} was assigned to ${campaign.name}.`,
      });
