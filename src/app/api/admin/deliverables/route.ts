...
const reviewManaged=new Set(['changes_requested','approved']);
...
export async function PATCH(request:Request){
  ...
  try{
    ...
    const patch:Record<string,unknown>={...b};if(patch.feedback==='')delete patch.feedback;if(patch.paymentReference==='')delete patch.paymentReference;if(b.paymentStatus==='paid'&&!b.paymentDate)patch.paymentDate=new Date().toISOString().slice(0,10);await updateDeliverable(id,patch);
