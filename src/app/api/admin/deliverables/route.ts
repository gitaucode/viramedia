...
export async function POST(request:Request){
  ...
  try{
    ...
    if(creatorId){
      const assignment=await db.prepare('SELECT 1 ok FROM shortlist_creators WHERE shortlist_id=? AND creator_id=?').bind(campaignId,creatorId).first<{ok:number}>();
    ...
    await recordActivity({
      ...
      detail:String(b.title||'New deliverable'),
      metadata:{dueDate:b.dueDate||null,status:b.status||'pending'},
    });
    if(creatorId){
...
export async function PATCH(request:Request){
  ...
  try{
    ...
    const before=await db.prepare(`SELECT d.id,d.title,d.creator_id,d.status,d.payment_status,s.id campaign_id,s.name campaign_name,c.full_name,c.email
      FROM deliverables d
      JOIN shortlists s ON s.id=d.campaign_id
      LEFT JOIN creators c ON c.id=d.creator_id
      WHERE d.id=?`).bind(id).first<{id:number;title:string;creator_id:number|null;status:string;payment_status:string;campaign_id:number;campaign_name:string;full_name:string|null;email:string|null}>();
    if(!before)return NextResponse.json({error:'Not found'},{status:404});

    const patch:Record<string,unknown>={...b};
    if(patch.feedback==='')delete patch.feedback;
    if(patch.paymentReference==='')delete patch.paymentReference;
    if(b.status==='approved'&&!b.approvedAt)patch.approvedAt=new Date().toISOString();
    if(b.paymentStatus==='paid'&&!b.paymentDate)patch.paymentDate=new Date().toISOString().slice(0,10);
    ...
    if(typeof b.status==='string'&&b.status!==before.status){
      await recordActivity({
        ...
        detail:`${before.title}: ${before.status.replaceAll('_',' ')} → ${b.status.replaceAll('_',' ')}`,
      ...
      await recordActivity({
        ...
        creatorId:before.creator_id,
        deliverableId:id,
        eventType:'deliverable.payment_changed',
        title:'Creator payment status changed',
        detail:`${before.title}: ${before.payment_status.replaceAll('_',' ')} → ${b.paymentStatus.replaceAll('_',' ')}`,
        metadata:{from:before.payment_status,to:b.paymentStatus},
      });
    }else{
      const changed=Object.keys(b).filter(key=>key!=='id');
      await recordActivity({
        actorType:'admin',
        campaignId:before.campaign_id,
        creatorId:before.creator_id,
    ...
    if(before.email&&typeof b.status==='string'&&b.status!==before.status&&['changes_requested','approved'].includes(b.status)){
      const origin=new URL(request.url).origin;
      const cleanFeedback=typeof b.feedback==='string'?b.feedback.trim():'';
      const feedbackHtml=cleanFeedback?`<p><strong>Feedback:</strong> ${cleanFeedback}</p>`:'';
      const isChanges=b.status==='changes_requested';
