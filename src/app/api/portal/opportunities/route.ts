...
export async function GET(){
 ...
 const opportunities=(r.results??[]).filter(o=>{
   if(o.application_status)return true;
   // all_approved bypasses matching but still requires approved portal access.
   const modeRow=o as Opportunity&{application_mode?:string};
   void modeRow;
 ...
 // Fetch modes separately to avoid leaking private campaigns while retaining simple D1 compatibility.
 const modes=await db.prepare(`SELECT id,application_mode FROM campaigns WHERE application_mode!='private'`).all<{id:number;application_mode:string}>();
 const modeMap=new Map((modes.results??[]).map(m=>[m.id,m.application_mode]));
 const visible=opportunities.filter(o=>modeMap.get(o.id)==='all_approved'||modeMap.get(o.id)==='matched');
