...
export async function POST(request:Request){
 try{
  ...
  const fullName=s(b.fullName,100),email=s(b.email,160),phone=s(b.phone,50),city=s(b.city,100),tiktok=s(b.tiktok,250);
  if(!fullName||!email.includes('@')||!phone||!city||!tiktok)return NextResponse.json({error:'Missing required fields'},{status:400});

  const saved=await saveCreatorApplication(b);
  ...
  // Once D1 is connected, the database is the source of truth. Email becomes a notification,
  // so a temporary email problem must not make a successfully stored application look lost.
